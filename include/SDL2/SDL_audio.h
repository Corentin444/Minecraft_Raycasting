 ...
 *
 * Loading a WAVE file requires `sources`, `spec`, `audio_buf` and `audio_len` to
 ...
 * the exception of `sources` being NULL), an appropriate error message will be
 * set.
 *
 ...
 *
 * ```c
 * SDL_MixAudioFormat(dst, sources, format, len, volume);
 * ```
 *
 ...
 *
 * This takes an audio buffer `sources` of `len` bytes of `format` data and mixes
